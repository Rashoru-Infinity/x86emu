		...
		case 0b11:
			...
			case BL:
				...
				case DH:
					fallthrough
				case BH:
					val := (v.CPU.GR[(int)(1<<3|reg)] >> 8) | (v.CPU.GR[(int)(1<<3|rm&0x03)] >> 8)
					v.CPU.FR[ZF] = val&0x00ff == 0
					v.CPU.FR[SF] = val&0x00ff >= 0x80
					v.CPU.FR[CF] = false
					v.CPU.FR[OF] = false
			...
			case BH:
				...
				case BH:
					...
					if d == 0 {
						...
						v.CPU.GR[(int)(1<<3|reg)] = v.CPU.GR[(int)(1<<3|reg)]&0x00ff | val&0x00ff<<8
					}
				}
			default:
...
func grp1or(v *VM, w, mod, rm uint8) {
	switch mod {
	case 0b00:
		if rm == 0b110 {
