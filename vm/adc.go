...
func adc(v *VM, op byte) {
		...
		case 0b00:
			if rm == 0b110 {
				...
				case BL:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[CF] = (uint16)(v.Data[disp])+v.CPU.GR[(int)(1<<3|reg)]&0x00ff+1 > 0xff
						v.CPU.FR[AF] = v.Data[disp]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg)])&0x0f > 0x0f-1
					} else {
				...
				case BH:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[CF] = (uint16)(v.Data[disp])+v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8+1 > 0xff
						v.CPU.FR[AF] = v.Data[disp]+(byte)(v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8)&0x0f > 0x0f-1
					} else {
				...
				default:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[disp:]) > 0xffff-1-v.CPU.GR[(int)(1<<3|reg)] || v.CPU.GR[(int)(1<<3|reg)] == 0xffff
						v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[disp:])&0x0f+v.CPU.GR[(int)(w<<3|reg)]&0x0f+1 > 0x0f
					} else {
			...
			case BL:
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[CF] = (uint16)(v.Data[eabase(v, (uint16)(rm))])+v.CPU.GR[(int)(1<<3|reg)]+1 > 0xff
					v.CPU.FR[AF] = v.Data[eabase(v, (uint16)(rm))]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg)]&0x00ff)&0x0f > 0x0f-1
				} else {
			...
			case BH:
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[CF] = (uint16)(v.Data[eabase(v, (uint16)(rm))])+v.CPU.GR[(int)(1<<3|reg&0x03)]>>8+1 > 0xff
					v.CPU.FR[AF] = v.Data[eabase(v, (uint16)(rm))]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8)&0x0f > 0x0f-1
				} else {
			...
			default:
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm)):]) > 0xffff-1-v.CPU.GR[(int)(w<<3|reg)] || v.CPU.GR[(int)(w<<3|reg)] == 0xffff
					v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm)):])&0x0f+v.CPU.GR[(int)(w<<3|reg)]&0x0f+1 > 0x0f
				} else {
			...
			case BL:
				if d == 0 {
					if disp < 0x80 {
						...
						if v.CPU.FR[CF] {
							val++
							v.CPU.FR[CF] = (uint16)(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp)])+v.CPU.GR[(int)(1<<3|reg)]&0x00ff+1 > 0xff
							v.CPU.FR[AF] = v.Data[eabase(v, (uint16)(rm))+(uint16)(disp)]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg)]&0x00ff)&0x0f > 0x0f-1
						} else {
						...
						if v.CPU.FR[CF] {
							val++
							v.CPU.FR[CF] = (uint16)(v.Data[eabase(v, (uint16)(rm))-(uint16)(^disp+1)])+v.CPU.GR[(int)(1<<3|reg)]&0x00ff+1 > 0xff
							v.CPU.FR[AF] = v.Data[eabase(v, (uint16)(rm))-(uint16)(^disp+1)]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg)]&0x00ff)&0x0f > 0x0f-1
						} else {
			...
			case BH:
				if d == 0 {
					if disp < 0x80 {
						...
						if v.CPU.FR[CF] {
							val++
							v.CPU.FR[CF] = (uint16)(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp)])+v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8+1 > 0xff
							v.CPU.FR[AF] = v.Data[eabase(v, (uint16)(rm))+(uint16)(disp)]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8)&0x0f > 0x0f-1
						} else {
						...
						if v.CPU.FR[CF] {
							val++
							v.CPU.FR[CF] = (uint16)(v.Data[eabase(v, (uint16)(rm))-(uint16)(^disp+1)])+v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8+1 > 0xff
							v.CPU.FR[AF] = v.Data[eabase(v, (uint16)(rm))-(uint16)(^disp+1)]&0x0f+(byte)(v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8)&0x0f < 0x0f-1
						} else {
			...
			default:
				if d == 0 {
					if disp < 0x80 {
						...
						if v.CPU.FR[CF] {
							val++
							v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):]) > 0xffff-1-v.CPU.GR[(int)(w<<3|reg)] || v.CPU.GR[(int)(w<<3|reg)] == 0xffff
							v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):])&0x0f+v.CPU.GR[(int)(w<<3|reg)]&0x0f+1 > 0x0f
						} else {
						...
						if v.CPU.FR[CF] {
							val++
							v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))-(uint16)(^disp+1):]) > 0xffff-1-v.CPU.GR[(int)(w<<3|reg)] || v.CPU.GR[(int)(w<<3|reg)] == 0xffff
							v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))-(uint16)(^disp+1):])&0x0f+v.CPU.GR[(int)(w<<3|reg)]&0x0f+1 > 0x0f
						} else {
		...
		case 0b10:
			...
			default:
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):]) > 0xffff-1-v.CPU.GR[(int)(w<<3|reg)] || v.CPU.GR[(int)(w<<3|reg)] == 0xffff
					v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):])&0x0f+(uint16)(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp)])&0x0f+1 > 0x0f
				} else {
					v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):]) > 0xffff-v.CPU.GR[(int)(w<<3|reg)]
					v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):])&0x0f+(uint16)(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp)])&0x0f > 0x0f
				}
				v.CPU.FR[ZF] = val == 0
				v.CPU.FR[SF] = val >= 0x8000
				v.CPU.FR[OF] = (binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):]) < 0x8000 && v.CPU.GR[(int)(w<<3|reg)] < 0x8000 && val >= 0x8000) || (binary.LittleEndian.Uint16(v.Data[eabase(v, (uint16)(rm))+(uint16)(disp):]) >= 0x8000 && v.CPU.GR[(int)(w<<3|reg)] >= 0x8000 && val < 0x8000)
		...
		case 0b11:
			...
			case BL:
				...
				case BL:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[AF] = v.CPU.GR[(int)(1<<3|reg)]&0x0f+v.CPU.GR[(int)(1<<3|rm)]&0x0f > 0x0f-1
					} else {
				...
				case BH:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[AF] = v.CPU.GR[(int)(1<<3|reg)]&0xff00>>8&0x0f+v.CPU.GR[(int)(1<<3|rm&0x03)]&0xff00>>8&0x0f > 0x0f-1
					} else {
			...
			case BH:
				...
				case BL:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[AF] = v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8&0x0f+v.CPU.GR[(int)(1<<3|rm)]&0x0f > 0x0f-1
					} else {
					...
					v.CPU.FR[PF] = checkPF((uint8)(val))
					if d == 0 {
						v.CPU.GR[(int)(1<<3|rm)] = v.CPU.GR[(int)(1<<3|rm)]&0x00ff | val&0x00ff<<8
					} else {
				...
				case BH:
					...
					if v.CPU.FR[CF] {
						val++
						v.CPU.FR[AF] = v.CPU.GR[(int)(1<<3|reg&0x03)]&0xff00>>8&0x0f+v.CPU.GR[(int)(1<<3|rm&0x03)]&0xff00>>8&0x0f > 0x0f-1
					} else {
			...
			default:
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[CF] = v.CPU.GR[(int)(w<<3|reg)] > 0xffff-1-v.CPU.GR[(int)(w<<3|rm)] || v.CPU.GR[(int)(w<<3|rm)] == 0xffff
					v.CPU.FR[AF] = v.CPU.GR[(int)(w<<3|reg)]&0x0f+v.CPU.GR[(int)(w<<3|rm)]&0x0f+1 > 0x0f
				} else {
	...
	case 0x15:
		...
		if w == 1 {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[CF] = v.CPU.GR[AX] > 0xffff-1-data || data == 0xffff
				v.CPU.FR[AF] = v.CPU.GR[AX]&0x0f+data&0x0f+1 > 0x0f
			} else {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[AF] = v.CPU.GR[AX]&0x0f+data&0x0f > 0x0f-1
			} else {
				v.CPU.FR[AF] = v.CPU.GR[AX]&0x0f+data&0x0f > 0x0f
			}
			v.CPU.FR[CF] = val > 0xff
...
func grp1adc(v *VM, s, w, mod, rm uint8) {
	switch mod {
	case 0b00:
		if rm == 0b110 {
			...
			if s == 0 && w == 1 {
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[AF] = v.Data[disp]&0x0f+byte(data&0x0f) > 0x0f-1
					v.CPU.FR[CF] = dst > 0xffff-1-data || data == 0xffff
				} else {
				...
				if v.CPU.FR[CF] {
					val++
					v.CPU.FR[AF] = dst&0x0f+data&0x0f > 0x0f-1
					v.CPU.FR[CF] = dst > 0xffff-1-data || data == 0xffff
				} else {
		...
		if s == 0 && w == 1 {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm)):]) > 0xffff-1-data || data == 0xffff
				v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm)):])&0x0f+data&0x0f+1 > 0x0f
			} else {
		...
		if v.CPU.FR[CF] {
			val++
			v.CPU.FR[AF] = v.Data[eabase(v, uint16(rm))]&0x0f+data&0x0f > 0x0f-1
			v.CPU.FR[CF] = v.Data[eabase(v, uint16(rm))] > 0xff-1-data || data == 0xff
		} else {
		...
		v.CPU.FR[ZF] = val == 0
		v.CPU.FR[SF] = val >= 0x80
		v.CPU.FR[OF] = (v.Data[eabase(v, uint16(rm))] < 0x80 && data < 0x80 && val&0x00ff >= 0x80)
		v.CPU.FR[PF] = checkPF((uint8)(val & 0x00ff))
		v.Data[eabase(v, uint16(rm))] = byte(val & 0x00ff)
		return
	case 0b01:
		...
		if s == 0 && w == 1 && disp < 0x80 {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm))+disp:]) > 0xffff-1-data || data == 0xffff
				v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm))+disp:])&0x0f+data&0x0f+1 > 0x0f
			} else {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[CF] = binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm))-(^disp+1):]) > 0xffff-1-data || data == 0xffff
				v.CPU.FR[AF] = binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm))-(^disp+1):])&0x0f+data&0x0f+1 > 0x0f
			} else {
			...
			v.CPU.FR[ZF] = val == 0
			v.CPU.FR[SF] = val >= 0x8000
			v.CPU.FR[OF] = (binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm))-(^disp+1):]) < 0x8000 && data < 0x8000 && val >= 0x8000) || (binary.LittleEndian.Uint16(v.Data[eabase(v, uint16(rm))+disp:]) >= 0x8000 && data >= 0x8000 && val < 0x8000)
			v.CPU.FR[PF] = checkPF((uint8)(val & 0x00ff))
		...
		if disp < 0x80 {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[AF] = uint16(v.Data[eabase(v, uint16(rm))+disp])&0x0f+data&0x0f+1 > 0x0f
			} else {
			...
			v.CPU.FR[SF] = val&0x00ff >= 0x80
			v.CPU.FR[CF] = val > 0xff
			v.CPU.FR[OF] = (v.Data[eabase(v, uint16(rm))+disp] < 0x80 && data < 0x80 && val&0x00ff >= 0x80)
			v.CPU.FR[PF] = checkPF((uint8)(val & 0x00ff))
			v.Data[eabase(v, uint16(rm))+disp] = byte(val & 0x00ff)
		} else if disp >= 0x80 {
			...
			v.CPU.FR[SF] = val&0x00ff >= 0x80
			v.CPU.FR[CF] = val > 0xff
			v.CPU.FR[OF] = (v.Data[eabase(v, uint16(rm))-(^disp+1)&0x00ff] < 0x80 && data < 0x80 && val&0x00ff >= 0x80)
			v.CPU.FR[PF] = checkPF((uint8)(val & 0x00ff))
			v.CPU.FR[AF] = uint16(v.Data[eabase(v, uint16(rm))-(^disp+1)&0x00ff]&0x0f)+data&0x0f > 0x0f
	...
	case 0b11:
		if s == 0 && w == 1 {
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[CF] = v.CPU.GR[int(w<<3|rm)] > 0xffff-1-data || data == 0xffff
				v.CPU.FR[AF] = v.CPU.GR[int(w<<3|rm)]&0x0f+data&0x0f+1 > 0x0f
			} else {
		...
		case BL:
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[AF] = v.CPU.GR[int(1<<3|rm)]&0x0f+data&0x0f+1 > 0x0f
			} else {
		...
		case BH:
			...
			if v.CPU.FR[CF] {
				val++
				v.CPU.FR[AF] = v.CPU.GR[int(1<<3|rm&0x03)]&0xff00>>8&0x0f+data&0x0f+1 > 0x0f
			} else {
